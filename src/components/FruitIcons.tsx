...
export function GrapesIcon(props: SVGProps<SVGSVGElement>) {
  // centered around x=32, fits within y 18-54 like other fruit icons
  const purple = [
    [27, 26], [22, 32], [32, 32], [17, 38], [27, 38], [37, 38],
    [22, 44], [32, 44], [27, 50],
  ];
  const green = [
    [37, 26], [32, 32], [42, 32], [27, 38], [37, 38], [47, 38],
    [32, 44], [42, 44], [37, 50],
  ];
  return (
    <svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" {...props}>
      ...
      <path d="M33 16c1 1 3 2 6 2" stroke="#3a5d2a" strokeWidth="1" fill="none" />
      {/* purple bunch (back) */}
      {purple.map(([x, y], i) => (
        <circle key={`p${i}`} cx={x} cy={y} r="5" fill="url(#grapePurple)" />
      ))}
      {/* green bunch (front, slightly offset right) */}
      {green.map(([x, y], i) => (
        <circle key={`g${i}`} cx={x} cy={y} r="5" fill="url(#grapeGreen)" opacity="0.92" />
      ))}
    </svg>
  );
}
